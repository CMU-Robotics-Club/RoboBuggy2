...
class Simulator:
    ...
    def __init__(self, heading: float):
        ...
        # To read from autogenerated velocity (currently set to a constant)
        self.simvelocity_subscriber = rospy.Subscriber(
            "buggy/velocity", Float64, self.update_velocity, "simulated"
        ) 
        # To read from manually entered velocity
        self.manvelocity_subscriber = rospy.Subscriber(
            "buggy/man_velocity", Float64, self.update_velocity, "manual"
        ) 
    ...
    #TODO: can i get this callback function to take in data from multiple subscribers and choose which one based on the state of a toggle?
    ...
    def update_velocity(self, data: Float64, source):
        ...
        """
        if (source == "simulated" and not Simulator.MANUAL_V_FLAG):
            with self.lock:
                self.velocity = data.data
        if (source == "manual" and Simulator.MANUAL_V_FLAG):
            with self.lock:
                self.velocity = data.data
