...
class Later_acc:
    ...
    def calculate_lateral_acc(self):
        if self.steering!=0:
            ...
            rear_flag_val = 0
            avg_flag_val = 0
            if lat_accel_front_wheel > self.flag_front:
                front_flag_val = 1
            if lat_accel_rear_wheel > self.flag_rear:
                rear_flag_val = 1
            if lat_accel > self.flag_avg:
                avg_flag_val = 1
            writer.writerow([time_rn, lat_accel_front_wheel,lat_accel_rear_wheel,lat_accel,front_flag_val,rear_flag_val,avg_flag_val])
